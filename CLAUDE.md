...
>
> **Not (2026-08-13):** Kullanıcı turuncu/terracotta vurgu rengini ("itici") ve 2026-08-10'da eklenen tamamen turuncu kalkan logosunu ("belirsiz bir karalama gibi görünüyor", özellikle favicon/navbar boyutunda) reddetti. **v1.13 olarak uygulandı**: vurgu rengi mavi (`--accent: #2563EB`), navbar zemini koyu kahveden beyaza döndü, logo aynı kavramla (kalkan + stetoskop + belge, kafa silüeti olmadan sadeleştirilmiş) yeniden üretildi. v1.13 canlıya alındı (`1007fd1`).
>
> **Not (2026-08-17):** Kullanıcı "eski turuncu halimize geri dönelim" dedi — **v1.13'ün tam tersi (v1.14), b49941f commit'inin reverse-patch'i uygulanarak yapıldı.** Vurgu rengi tekrar turuncuya (`--accent: #DD5C22`) döndü, navbar logosu tekrar `static/logo-mark-koyu.png`'a bağlandı, favicon/apple-touch-icon binary'leri b49941f'in bir önceki (turuncu, yuvarlak köşe) haline geri yüklendi. `static/logo-mark-blue.png` **silinmedi** — proje konvansiyonu gereği (bkz. eski `logo-mark-koyu.png` emsali) kullanılmayan eski tasarım dosyaları repoda bırakılıyor, ileride mavi tekrar istenirse hazır duruyor. **Navbar zemini** ise revert'in aynı gününde ayrı bir istekle koyu kahveden (`rgba(36,26,16,0.92)`) **açık kahverengiye/tan'a** (`rgba(201,168,118,0.92)`) çevrildi — dolayısıyla v1.14, v1.13-öncesi orijinal turuncu döneminin birebir aynısı değil, navbar zemini hem eski koyu kahveden hem yeni mavi-dönem beyazından farklı, üçüncü bir ton. Navbar metin renkleri buna göre koyu ink'e (`var(--ink)`) çevrildi (beyaz/krem yazı açık zeminde okunmaz olurdu). Yerelde masaüstü+mobil (CDP) doğrulandı, henüz deploy edilmedi — bkz. `memory/checkpoint-son.md`.

## 1. UI/UX Tasarım Felsefesi (v1.16 — Sıcak/Samimi Yön + Turuncu Marka Rengi + Gradyanlı "Göz/CD" Logosu)
...
- **Uygulanan yön (canlıda, `templates/index.html`):**
  - Palet: krem/kağıt zemin (`--paper` ≈ `#FBF5EB`) + **terracotta turuncu vurgu** (`--accent: #DD5C22`, `--accent-dark: #B3450F`, `--accent-soft: #FBE3CE`, `--accent-softer: #FCEEE1`) + sıcak kahve koyu tonlar (`--dark`/`--dark-2`, ≈ `#241A10`/`#38281A`, demo/premium/loading/rapor kartı başlıkları gibi iç bileşenlerde). **Navbar zemini kendi ayrı tonunu kullanıyor:** `rgba(140,108,74,0.92)` (orta-açık kahverengi, token değil hard-code — henüz bir `--navbar-bg` değişkenine bağlanmadı) + **beyaz metin** (`#FFFFFF` marka adı ve linkler, hover `var(--accent-soft)`). Bu ton, beyaz yazının okunabilmesi için bilinçli olarak orijinal çok koyu `--dark` (#241A10) ile önceki "açık tan" denemesi (rgba(201,168,118), o tonda beyaz yazı ~2.25:1 kontrastla okunmuyordu) arasında seçildi — beyaz üzerinde ~4.8:1 (WCAG AA geçer). Mavi (`#2563EB` ailesi, `--accent-light`/`--accent-rgb` token'ları) artık **hiçbir yerde yok** — 2026-08-17'de tekrar turuncuya döndürüldü (v1.13'ün geri alınması, bkz. yukarıdaki not).
  ...
  - **Ürün akış GIF'i** (`static/urun-akisi.gif`) hero'da — gerçek DOM/CSS/JS'ten üretilmiş, dürüstçe "örnek CV/ilan metniyle" etiketlenmiş, sahte "canlı analiz" iddiası yok. GIF içindeki "Analizi Başlat" butonu 2026-07-24'te turuncu paletle üretilmişti; v1.13'te (mavi dönemde) bu bir tutarsızlıktı, **v1.14'e dönüşle bu tutarsızlık kendiliğinden çözüldü** (GIF zaten turuncuydu).
  - **Logo/favicon (2026-08-17'de iki kez değişti — v1.16, güncel):** Kalkan+stetoskop+baş silüeti konsepti terk edildi. Aynı "göz + CD monogramı" fikrinin **iki farklı Gemini üretimi** denendi: ilki düz tek renk (`#DD5C22`), kısa süre canlıya alındı; kullanıcı ardından **gradyanlı** (açık turuncudan koyu kızıl-turuncuya) ikinci bir üretim ekledi ve "onu kullan" dedi — bu güncel/canlı versiyon. **Not:** Bu gradyan, projenin kendi `docs/logo-brief.md`'sindeki "gradient, 3D, gölge, parlaklık yok — sistem düz ve editoryal" kuralını teknik olarak ihlal ediyor; kullanıcıya belirtildi, kullanıcı bilinçli olarak bu yönü tercih etti. Kaynak dosya `static/Gemini_Generated_Image_brbn5rbrbn5rbrbn.jpg` (2048px, düz beyaz zemin — öncekinin aksine kağıt dokusu yok) PIL ile işlendi: R-B kanal farkına göre alfa maskesi çıkarıldı, içerik bbox'ına göre kare kırpıldı, orijinal gradyan renkleri korunarak (tek renge düzleştirilmeden) `MaxFilter` ile hafif kalınlaştırıldı. Bu ikinci tasarımın çizgileri zaten daha kalın olduğu için 16-32px'te ilk versiyona göre daha az post-processing gerekti. Üç türev: `static/favicon-32.png` (32px, beyaz zemin, ~%22 yuvarlak köşe), `static/apple-touch-icon.png` (180px, aynı stil), `static/logo-mark-goz.png` (navbar, **kullanıcının isteğiyle orijinal turuncu/gradyan renginde** — ilk versiyonda kontrast sorunu nedeniyle beyaza çevrilmişti (~1.3:1), kullanıcı "kendi orijinal renginde olsun" dedi, bu ikinci tasarımın daha parlak/kalın çizgileri navbar'ın kahverengi zemininde ilkine göre gözle görülür ölçüde daha okunaklı çıktı, ayrıca kontrast ölçülmedi). Düz turuncu/gradyanlı master `static/logo-goz.png` olarak da tutuluyor. Eski kalkan dosyaları (`logo-mark-koyu.png` vb.) ve ilk göz/CD üretiminin kaynağı (`Gemini_Generated_Image_841hkm841hkm841h.jpg`) silinmedi, konvansiyon gereği arşivde duruyor. `static/og-image.png` hâlâ eski (2026-07 öncesi) stetoskop+ok markasını kullanıyor — hâlâ açık iş.
  ...
  - **Kapsam dışı bırakılan renkler:** premium teklif kartının altın/amber rengi (`#FBBF24`/`#F59E0B` — ayrı bir "özel paket" sinyali), `--danger`/`--success` semantik renkleri, Türk bayrağı kırmızısı. Bunlar turuncu↔mavi geçişlerinin hiçbirinden etkilenmedi.
