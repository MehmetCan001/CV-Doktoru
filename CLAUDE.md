...
## 1. UI/UX Tasarım Felsefesi (v1.15 — Sıcak/Samimi Yön + Turuncu Marka Rengi + Yeni "Göz/CD" Logosu)
...
- **Uygulanan yön (canlıda, `templates/index.html`):**
  ...
  - **Logo/favicon (2026-08-17'de tekrar değişti — v1.15):** Kalkan+stetoskop+baş silüeti konsepti terk edildi. Kullanıcı Gemini ile yeni bir mark ürettirdi: **göz şekli içinde "CD" (CV Doktoru) monogramı**, tek renk düz vektör, klişe (stetoskop/tıp haçı/kafa) yok — `docs/logo-brief.md`'nin "istemediklerimiz" listesiyle nihayet uyumlu. Kaynak dosya `static/Gemini_Generated_Image_841hkm841hkm841h.jpg` (2048px, kağıt dokulu krem zemin) PIL ile işlendi: R-B kanal farkına göre alfa maskesi çıkarıldı (arka plan dokusu şeffaflaştı), içerik bbox'ına göre kare kırpıldı, `MaxFilter` ile çizgiler kalınlaştırıldı (16-32px'te ince "CD" detayının kaybolmaması için — bu projede tekrar eden bir ders, bkz. Bölüm 12). Üç ayrı türev üretildi: `static/favicon-32.png` (32px, beyaz zemin, yuvarlak köşe ~%22 radius, turuncu `#DD5C22`), `static/apple-touch-icon.png` (180px, aynı stil, daha hafif kalınlaştırma çünkü büyük boyutta ince detay zaten okunuyor), `static/logo-mark-goz.png` (navbar, **beyaz** — turuncu değil: navbar zemini `rgba(140,108,74)` ile turuncu ikon arası ölçülen kontrast ~1.3:1 çıktı, yetersizdi; beyaz ikon aynı navbar metniyle aynı ~4.8:1 kontrastı taşıyor). Düz turuncu master `static/logo-goz.png` olarak da tutuluyor (gelecekte baskı/PDF/og-image için). Eski kalkan dosyaları (`logo-mark-koyu.png`, `logo-kalkan.png` vb.) silinmedi, konvansiyon gereği arşivde duruyor ama artık hiçbir yerden referans edilmiyor. `static/og-image.png` hâlâ eski (2026-07 öncesi) stetoskop+ok markasını kullanıyor — bu değişiklikten de etkilenmedi, hâlâ açık iş.
