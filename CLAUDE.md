...
## 1. UI/UX Tasarım Felsefesi (v1.14 — Sıcak/Samimi Yön + Turuncu Marka Rengi, tekrar)
...
- **Uygulanan yön (canlıda, `templates/index.html`):**
  ...
  - **Logo/favicon:** Kalkan + stetoskop + belge kavramı, kafa silüetli turuncu versiyon — `static/logo-mark-koyu.png` (navbar, koyu zeminde), `static/favicon-32.png`, `static/apple-touch-icon.png` (yuvarlak köşe, `#F28A4E` tonu). Mavi/kafa-siluetsiz versiyon (`static/logo-mark-blue.png`) dosyası repoda duruyor ama şu an hiçbir yerden referans edilmiyor. `static/og-image.png` (paylaşım banner'ı) hâlâ daha eski (2026-07 öncesi) stetoskop+ok markasını kullanıyor — bilinen açık iş, bu revert'ten etkilenmedi.
